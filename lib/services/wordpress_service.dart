...
class WordPressService {
  Future<List<Article>> fetchPostsByCategoryIdExceptPostId(
      int? postId, int? catId, int contentAmount) async {
    List<Article> articles = [];
    final String url =
        "${WpConfig.baseURL}/wp-json/wp/v2/posts?exclude=$postId&categories[]=$catId&per_page=$contentAmount";
    try {
  ...
  Future<List<Article>> fetchPostsByCategoryId(
      int? catId, int page, int contentAmount) async {
    List<Article> articles = [];
    final String url =
        "${WpConfig.baseURL}/wp-json/wp/v2/posts?categories[]=$catId&page=$page&per_page=$contentAmount";
    try {
      var response =
  ...
  Future<List<Article>> fetchAllPosts(
      int page, int contentAmount, List<int> blockedCategoryIds) async {
    String ids = AppService.getIds(blockedCategoryIds);
    List<Article> articles = [];
    final String url =
        "${WpConfig.baseURL}/wp-json/wp/v2/posts/?page=$page&per_page=$contentAmount&categories_exclude=$ids";
    try {
      var response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        List decodedData = jsonDecode(response.body);
  ...
  Future<List<Article>> fetchPostsBySearch(
      String searchText, List<int> blockedCategoryIds) async {
    List<Article> articles = [];
    String ids = AppService.getIds(blockedCategoryIds);
    final String url =
        "${WpConfig.baseURL}/wp-json/wp/v2/posts?per_page=30&search=$searchText&categories_exclude=$ids";
    try {
      var response =
          await http.get(Uri.parse(url)).timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
  ...
  Future<Article?> getPostBySlug(String slug) async {
    ...
    try {
      var response = await http
          .get(Uri.parse("${WpConfig.baseURL}/wp-json/wp/v2/posts?slug=$slug"));
      if (response.statusCode == 200) {
        List decodedData = jsonDecode(response.body);
        List<Article> articles =
            decodedData.map((e) => Article.fromJson(e)).toList();
  ...
  Future<Article?> fetchPostsById(int postID) async {
    ...
    try {
      var response = await http
          .get(Uri.parse("${WpConfig.baseURL}/wp-json/wp/v2/posts/$postID"));
      if (response.statusCode == 200) {
  ...
  Future<List<Article>> fetchPostsByTag(
      int pageNumber, int tagID, int contentAmount) async {
    final String url =
        '${WpConfig.baseURL}/wp-json/wp/v2/posts/?tags=$tagID&page=$pageNumber&per_page=$contentAmount';
    List<Article> articles = [];
    try {
      final response =
          await http.get(Uri.parse(url)).timeout(const Duration(seconds: 10));
  ...
  Future<List<Article>> fetchVideoPosts(int page, int postAmount) async {
    final String url =
        '${WpConfig.baseURL}/wp-json/wp/v2/posts?video=1&page=$page&per_page=$postAmount';
    List<Article> articles = [];
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
  ...
  Future<List<Article>> fetchPostsByAuthor(
      int pageNumber, int authorId, int contentAmount) async {
    final String url =
        '${WpConfig.baseURL}/wp-json/wp/v2/posts/?page=$pageNumber&author=$authorId&status=publish&per_page=$contentAmount';
    List<Article> articles = [];
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
  ...
  Future<List<CommentModel>> fetchCommentsByPostId(
      int id, int page, int amount) async {
    List<CommentModel> comments = [];
    try {
      var response = await http.get(Uri.parse(
          "${WpConfig.baseURL}/wp-json/wp/v2/comments?post=$id&page=$page&per_page=$amount"));
  ...
  Future<bool> postComment(
    ...
    try {
      var response = await http
          .post(Uri.parse("${WpConfig.baseURL}/wp-json/wp/v2/comments"), body: {
  ...
  Future<ConfigModel?> getConfigsFromAPI() async {
    ...
    try {
      var response = await http
          .get(Uri.parse("${WpConfig.baseURL}/wp-json/newsfreak/configs"))
  ...
  Future<List<Category>> fetchCategoriesByIDs(List ids) async {
    ...
    if (ids.isEmpty) {
      ...
      final String url =
          '${WpConfig.baseURL}/wp-json/wp/v2/categories?include=$categoryIds';
      try {
        final response = await http.get(Uri.parse(url));
        if (response.statusCode == 200) {
  ...
  Future<List<Article>> fetchPopularPosts(
      String timeRange, int contentAmount) async {
    List<Article> articles = [];

    final String url =
        "${WpConfig.baseURL}/wp-json/wordpress-popular-posts/v1/popular-posts?range=$timeRange&limit=$contentAmount";
  ...
  static Future<bool> addViewsToPost(int postID) async {
    final String url =
        '${WpConfig.baseURL}/wp-json/wordpress-popular-posts/v1/popular-posts?wpp_id=$postID';
    try {
  ...
  Future<List<Category>> getCategories(List<int> blockedCategoryIds) async {
    ...
    List<Category> categories = [];
    final String url =
        "${WpConfig.baseURL}/wp-json/wp/v2/categories?per_page=100&order=desc&orderby=count&exclude=$ids";
    try {
      var response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        List? decodedData = jsonDecode(response.body);
  ...
  Future<bool> postCommentReply(int postId, String userName, String userEmail,
      String comment, int parentId) async {
    try {
      var response = await http
          .post(Uri.parse("${WpConfig.baseURL}/wp-json/wp/v2/comments"), body: {
